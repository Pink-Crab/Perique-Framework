...
class Dice {
	 ...
	 */
	public function create( string $name, array $args = array(), array $share = array() ) {
	...
	private function getClosure( string $name, array $rule ) {
		...
		if ( isset( $rule['shareInstances'] ) ) {
			$closure = function( array $args, array $share ) use ( $closure, $rule ) {
				foreach ( $rule['shareInstances'] as $instance ) {
					$share[] = $this->create( $instance, array(), $share );
				}
		...
		return isset( $rule['call'] ) ? function ( array $args, array $share ) use ( $closure, $class, $rule, $name ) {
			...
			foreach ( $rule['call'] as $call ) {
				...
				$params = $this->getParams( $class->getMethod( $call[0] ), array( 'shareInstances' => isset( $rule['shareInstances'] ) ? $rule['shareInstances'] : array() ) )( ( $this->expand( isset( $call[1] ) ? $call[1] : array() ) ), $share );
	 ...
	 * @return mixed
	 */
	private function expand( $param, array $share = array(), bool $createFromString = false ) {
		if ( is_array( $param ) ) {
	...
	private function getParams( ReflectionMethod $method, array $rule ) {
		...
		return function ( array $args, array $share = array() ) use ( $paramInfo, $rule ) {
			...
			foreach ( $paramInfo as list($class, $param, $sub) ) {
				...
				elseif ( $share && ( $match = $this->matchParam( $param, $class, $share ) ) !== false ) {
					$parameters[] = $match;
				}
